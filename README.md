...
<div style='font-size:13px;opacity:0.92;'>Lineage heat: 1 outbound / 3 inbound · ✅ No engine drift detected · Index coverage: ready · PII: 3 · Changelog: fresh (2 d)</div>
</div>
...
- Pkg folder: [packages\key-wrappers](https://github.com/blackcatacademy/blackcat-database/blob/main/packages\key-wrappers)
...
graph LR
  ...
  key_wrappers["key_wrappers"]:::center
  key_wrappers -->|FK| kms_keys["kms_keys"]:::link
  encryption_bindings["encryption_bindings"]:::inbound -->|FK| key_wrappers
  key_wrapper_layers["key_wrapper_layers"]:::inbound -->|FK| key_wrappers
  rewrap_jobs["rewrap_jobs"]:::inbound -->|FK| key_wrappers
...
- Outbound (depends on): "kms_keys"
- Inbound (relies on this): "encryption_bindings", "key_wrapper_layers", "rewrap_jobs"
...
## Usage
- Install/upgrade schema: pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package key-wrappers -Apply
