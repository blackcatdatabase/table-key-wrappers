...
namespace BlackCat\Database\Packages\KeyWrappers\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto;
use BlackCat\Database\Packages\KeyWrappers\Mapper\KeyWrapperDtoMapper;
 ...
 */
final class KeyWrappersAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private KeyWrapperRepository $keyWrapperRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'key_wrappers' => $this->keyWrapperRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\KeyWrappers\ModuleException("Record {$idStr} not found for locking.");
            }
