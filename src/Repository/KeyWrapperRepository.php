...
use BlackCat\Database\Packages\KeyWrappers\Repository\KeyWrapperRepositoryInterface;
...
class KeyWrapperRepository implements KeyWrapperRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'wrapper_uuid' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\KeyWrappers\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'kms1_key_id', 'kms2_key_id', 'dek_wrap1', 'dek_wrap2', 'crypto_suite', 'wrap_version', 'status', 'rotated_at' ]);
          return count($rows);
        }
    ...
    public function getByWrapperUuid(string $wrapperUuid, bool $asDto = false): array|\BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto|null {
        return $this->getByUnique([ 'wrapper_uuid' => $wrapperUuid ], $asDto);
    }
    ...
    public function getIdByWrapperUuid(string $wrapperUuid) {
        $row = $this->getByWrapperUuid($wrapperUuid, false);
    ...
    public function getByKms1KeyIdAndKms2KeyIdAndWrapVersion(int $kms1KeyId, int $kms2KeyId, int $wrapVersion, bool $asDto = false): array|\BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto|null {
        return $this->getByUnique([ 'kms1_key_id' => $kms1KeyId, 'kms2_key_id' => $kms2KeyId, 'wrap_version' => $wrapVersion ], $asDto);
    }
    ...
    public function getIdByKms1KeyIdAndKms2KeyIdAndWrapVersion(int $kms1KeyId, int $kms2KeyId, int $wrapVersion) {
        $row = $this->getByKms1KeyIdAndKms2KeyIdAndWrapVersion($kms1KeyId, $kms2KeyId, $wrapVersion, false);
        if (!is_array($row)) { return null; }
        return $row['id'] ?? null;
    }
