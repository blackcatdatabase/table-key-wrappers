...
namespace BlackCat\Database\Packages\KeyWrappers\Repository;

use BlackCat\Core\Database as Database;              // e.g. BlackCat\Core\Database
use BlackCat\Database\Packages\KeyWrappers\Definitions;
use BlackCat\Database\Packages\KeyWrappers\Criteria;
use BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto as Dto;
use BlackCat\Database\Packages\KeyWrappers\Mapper\KeyWrapperDtoMapper as RowMapper;
...
final class KeyWrapperRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\KeyWrappers\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'kms1_key_id', 'kms2_key_id', 'dek_wrap1', 'dek_wrap2', 'crypto_suite', 'wrap_version', 'status', 'rotated_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'wrapper_uuid' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\KeyWrappers\Definitions::table());
            return $bulk->upsertMany($rows, [ 'wrapper_uuid' ], [ 'kms1_key_id', 'kms2_key_id', 'dek_wrap1', 'dek_wrap2', 'crypto_suite', 'wrap_version', 'status', 'rotated_at' ]);
        }
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto|null */
    public function getByWrapperUuid(string $wrapperUuid, bool $asDto = false): array|\BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto|null {
        $row = $this->getByUnique([ 'wrapper_uuid' => $wrapperUuid ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KeyWrappers\Mapper\KeyWrapperDtoMapper::fromRow($row);
    }
    public function existsByWrapperUuid(string $wrapperUuid): bool {
        $where = 't.' . Ident::q($this->db, 'wrapper_uuid') . ' = :uniq_wrapper_uuid';
        return $this->exists($where, [ 'uniq_wrapper_uuid' => $wrapperUuid ]);
    }
    /** @return int|string|null */
    public function getIdByWrapperUuid(string $wrapperUuid) {
        $row = $this->getByWrapperUuid($wrapperUuid);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto|null */
    public function getByKms1KeyIdAndKms2KeyIdAndWrapVersion(int $kms1KeyId, int $kms2KeyId, int $wrapVersion, bool $asDto = false): array|\BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto|null {
        $row = $this->getByUnique([ 'kms1_key_id' => $kms1KeyId, 'kms2_key_id' => $kms2KeyId, 'wrap_version' => $wrapVersion ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\KeyWrappers\Mapper\KeyWrapperDtoMapper::fromRow($row);
    }
    public function existsByKms1KeyIdAndKms2KeyIdAndWrapVersion(int $kms1KeyId, int $kms2KeyId, int $wrapVersion): bool {
        $where = 't.' . Ident::q($this->db, 'kms1_key_id') . ' = :uniq_kms1_key_id' . ' AND ' . 't.' . Ident::q($this->db, 'kms2_key_id') . ' = :uniq_kms2_key_id' . ' AND ' . 't.' . Ident::q($this->db, 'wrap_version') . ' = :uniq_wrap_version';
        return $this->exists($where, [ 'uniq_kms1_key_id' => $kms1KeyId, 'uniq_kms2_key_id' => $kms2KeyId, 'uniq_wrap_version' => $wrapVersion ]);
    }
    /** @return int|string|null */
    public function getIdByKms1KeyIdAndKms2KeyIdAndWrapVersion(int $kms1KeyId, int $kms2KeyId, int $wrapVersion) {
        $row = $this->getByKms1KeyIdAndKms2KeyIdAndWrapVersion($kms1KeyId, $kms2KeyId, $wrapVersion);
        return $row ? ($row['id'] ?? null) : null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\KeyWrappers\Dto\KeyWrapperDto|null {
